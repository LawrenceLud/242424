...
-- Criando o Toggle de Auto Relics
...
AutoRelics:OnChanged(function(enabled)
    ...
    -- Loop para teleportar para os objetos gerados
    task.spawn(function()
        while enabled and running do
            -- Verifica se o toggle ainda está ativado
            if not AutoRelics.Value then 
                running = false 
                break 
            end 
            
            -- Itera sobre os objetos gerados
            for _, relic in pairs(workspace:WaitForChild("Game"):WaitForChild("Trinkets"):WaitForChild("Spawned"):GetChildren()) do
                if relic:IsA("Model") and relic.PrimaryPart then
                    humanoidRootPart.CFrame = relic.PrimaryPart.CFrame + Vector3.new(0, 3, 0) -- Teleporta um pouco acima
                    task.wait(0.5) -- Pequena espera para evitar bugs
                end
            end
            task.wait(1) -- Tempo entre verificações
        end
    end)
end)
